...
class HistoryView(ctk.CTkFrame):
    ...
    def __init__(self, parent: ctk.CTkFrame, app: MeetScribeApp) -> None:
        ...
        self._search_entry = ctk.CTkEntry(search_frame, placeholder_text="Поиск по встречам...")
        self._search_entry.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        self._search_entry.bind("<Return>", lambda e: self._refresh())

        ctk.CTkButton(search_frame, text="Найти", width=80, command=self._refresh).grid(row=0, column=1)
    ...
    def _refresh(self) -> None:
        ...
        if not meetings:
            ctk.CTkLabel(self._scroll, text="Нет встреч", text_color="gray").grid(row=0, column=0, pady=40)
            return
        ...
        for i, m in enumerate(meetings):
            ...
            title_label = ctk.CTkLabel(card, text=m.title or "Без названия", anchor="w", font=ctk.CTkFont(weight="bold"))
